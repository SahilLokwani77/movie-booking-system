...
export default function MovieBookingApp() {
  ...
  const confirmBooking = () => {
    const booking = {
      id: Date.now(),
      key: `${selectedMovie.id}-${selectedTheatre.id}-${selectedDate}-${selectedShow}`,
      movie: selectedMovie,
  ...
  try {
    ...
    // 👉 backend se updated list lao
    const getRes = await fetch("http://localhost:5000/api/bookings");
    const allBookings = await getRes.json();
    setBookings(allBookings);

    showToast("🎉 Booking Saved to DB!");
              ...
              <div style={{ position: "fixed", inset: 0, background: "#000a", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
                <div style={{ background: "#1a1a2e", border: "1px solid #3a2a5a", borderRadius: 16, padding: "2rem", width: 380 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button style={{ ...styles.btn, ...styles.btnSuccess, flex: 1 }} onClick={confirmBooking}>Pay ₹{totalAmount}</button>
                  </div>
